...
export type LiveResult = { text: string; costUsd: number; model: string; usage: { input_tokens: number; output_tokens: number } };
const system = "Return only JSON: {\"summary\":\"...\",\"files\":{\"out/path\":\"contents\"}}.";

export async function callLiveModel(prompt: string): Promise<LiveResult> {
  const executor = flag("--executor") ?? process.env.MINI_EXECUTOR ?? "claude-cli";
  if (executor === "claude-cli") return callClaude(prompt);
  ...
  const agent = new Agent({ model: createModel(provider, modelId), tools: [], systemPrompt: system, printer: false });
  ...
  return { text, usage, model: `${provider}:${modelId}`, costUsd: estimate(usage) };
}
...
function callClaude(prompt: string): Promise<LiveResult> {
