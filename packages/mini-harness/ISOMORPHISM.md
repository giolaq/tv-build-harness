...
| `executor.ts` | `packages/harness/src/executors/{agent-sdk,claude-cli,strands}.ts` | Puts replay, local Claude Code, or remote Strands behind an interface. | More phase controls, streaming, tool use, provider overrides, usage accounting, and recording taps. |
...
| `skills.ts` | `packages/harness/src/skill-library.ts` | Loads named Markdown skills for executor-specific delivery. | Stack-aware routing, richer validation, auto-skills, and quality gates; the production Strands path also builds `AgentSkills` in `phase-context.ts`. |
