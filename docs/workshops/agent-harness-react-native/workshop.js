...
const modules = [
  ...
  },
  {
    ...
    body: `${concept("Why this step exists","A model can produce plausible files, but plausibility is not evidence. Start with the smallest possible agent so its missing guarantees are easy to see.")}${predict("Before you run it, name one bug that could hide inside output that looks complete.")}${command("Run Step 1 with replay","step1")}<h2>Trace the model boundary</h2>${steps(["Open <code>steps/01-single-agent/index.ts</code>.","Find the prompt, model response, and file-writing code.","Inspect the generated files. Do not judge them only by appearance.","Write down three claims that need an independent check."])}${knowledgeCheck("Why is this an agent script, but not yet a reliable harness?","It has a model call and side effects, but no independent verification, bounded retry, checkpoint, approval gate, or durable evidence.")}${done("You can point to the model boundary and name three missing checks.")}${fallback("The replay is the complete exercise. No live model is needed.")}`
  },
  {
    ...
    body: `${concept("Why this step exists","A retry is useful only when it carries new information. The check turns a vague failure into precise context the next attempt can act on.")}${predict("The recording contains two model responses. Predict which requirement the first response misses.")}${command("Replay the failed check and repair","step2")}${expected(`Pattern "Kitchen Stories" not found`)}<h2>Follow the evidence</h2>${steps(["Find the failed <code>grep</code> check in the output.","Open <code>steps/02-verify-loop/verify.ts</code> and locate that check.","Find the same failure text in the retry request.","Confirm the second attempt passes the original check."])}${knowledgeCheck("Why pass the exact failure into the retry instead of saying try again?","The exact failure narrows the problem, preserves the original requirement, and makes the retry explainable. A generic retry mostly buys another guess.")}${done("You can trace requirement → failed check → retry → passing result.")}${fallback("Use the committed retry recording. Do not replace this exercise with a live call.")}`
  },
  {
    ...
    evidence: "A paused run resumes at polish, while the Git log keeps one commit per completed phase.",
    body: `${concept("Why this step exists","Long agent runs fail. Small phases limit the damage, checkpoints remember orchestration progress, and commits preserve code that already passed its checks.")}${predict("The run stops after content. Which phase should resume next, and which phases must not run again?")}${command("Pause after the content phase","step3")}${expected(`Paused after content.\ncheckpoint.json: { "nextPhase": 2 }`)}<h2>Inspect before resuming</h2>${steps(["Open <code>out/checkpoint.json</code>.","Use <code>phases.json</code> to confirm index 2 is <code>polish</code>.","Open the Git log and find commits for scaffold and content."])}${command("Resume the same run","step3Resume")}<h2>Compare the result</h2>${steps(["Confirm only polish ran after resume.","Open the final checkpoint and Git log.","Explain what progress belongs in the checkpoint and what evidence belongs in Git."])}${knowledgeCheck("Why keep both a checkpoint and per-phase commits?","The checkpoint tells the engine where to continue. Git records the exact verified code state for each completed phase. They answer different recovery questions.")}${done("The second command runs only polish, without repeating scaffold or content.")}${fallback("Read <code>fixtures/resume/README.md</code>, then repeat the two replay commands.")}`
  },
  {
    ...
    body: `${concept("Four responsibilities","Skills teach domain knowledge. Phase context assembles the task. An executor talks to a model. Tools expose narrow capabilities. The pipeline decides when side effects are allowed.")}${predict("Where should a D-pad focus rule live: the executor, a skill, a read tool, or a verification check?")}${command("Run Step 4 with replay","step4")}<h2>Map the teaching harness</h2>${steps(["Open <code>skills.ts</code>, <code>phase-context.ts</code>, <code>executor.ts</code>, and <code>recorder.ts</code>.","Find where skill text enters the prompt.","Find the common interface used by replay, Claude Code, and Strands.","Compare every module with <code>packages/mini-harness/ISOMORPHISM.md</code>."])}<h2>Inspect the production Strands boundary</h2>${steps(["Open <code>packages/workshop-harness/src/port-tools.ts</code> and find the Zod-typed read tools.","Open <code>port-contract.ts</code> and find the validated patch schema.","Open <code>port-executor.ts</code> and find the turn, token, and time limits.","Confirm the agent has no write or shell tool. The pipeline owns both."])}${knowledgeCheck("Why keep file writes outside the Strands agent?","The harness can validate a typed proposal before applying it, protect paths, produce a clean diff, run checks, and roll back safely. The model still reasons; deterministic code controls the irreversible action.")}<h2>Optional live comparison</h2>${command("Use local Claude Code","step4Local")}${command("Use Strands with Bedrock","step4Remote")}${done("You can change model providers without changing checks, and explain why the agent has read-only tools.")}${fallback("Replay shows the same module boundaries without credentials.")}`
  },
  {
