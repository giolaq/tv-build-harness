...
The harness doesn't just generate code — it iterates. It builds, screenshots, analyzes with vision, fixes bugs, rebuilds, and re-tests until the app meets quality standards. And it learns: fixes discovered in one run become skills that prevent the same bug in future runs.
...
- Auto-skillify: the harness learns from its own mistakes
...
### Auto-Skillify (Self-Improving System)

The harness learns from its mistakes:

1. Agent encounters a bug during a code phase
2. After fixing it, agent evaluates: "Was this reusable?" (self-scores 1-5)
3. If score ≥ 4, writes a new skill to `skills/auto/`
4. Tracks effectiveness: `times_loaded` / `times_defect_recurred`
5. After 3+ loads with 0 recurrence → suggest promoting to core prompt
6. CLI commands: `prune-skills`, `consolidate-skills`
...
**Note**: This deck is from an earlier stage (pre-verification, pre-multi-provider). A new presentation should incorporate the verification system, multi-provider support, the bug stories, auto-skillify, the course material, and the 8-week journey narrative.
...
## Auto-Generated Skills (Living Proof of Self-Improvement)

Two skills were auto-generated by the harness during actual runs — concrete evidence of the self-improving system:
...
| Reusable knowledge across runs | ≥ 1 skill auto-written per novel task class | Yes (auto-skillify system) |
...
- Agents can ONLY write to: generated app (`out/<runId>/app/`) + `skills/auto/`
...
3. **The feedback loops**: visual QA, device testing, auto-skillify — three different learning loops
